 ...
 * # wall  = support  . floor  ~ nukage  D door  S secret door (looks like wall)
 * P player  E enemy  X exit
 * 1 pea ammo  2 corn shotgun  3 cob launcher  H health  A armor
 */
...
export const LEVELS = [
  {
    ...
    name: "Hangar",
    /*
     * Romero Hangar horseshoe — oriented like the classic top-down map:
     * WEST start + armor alcove → CENTER computer (hex nukage + megaarmor)
     * → EAST zig-zag nukage → SOUTH exit. Secret wall → shotgun alcove.
     */
    grid: [
      "########################################################",
      "#......................................................#",
...
export function parseLevel(def) {
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      ...
      let type = 0;
      if (c === "#" || c === "=") type = 1;
      else if (c === "D" || c === "S") type = 2;
      else if (c === "~") type = 3;
      tiles[y][x] = type;

      if (c === "S") wallTex[y][x] = "stone";
      else if (c === "D") wallTex[y][x] = "door";
      ...
      if (c === "D" || c === "S") {
        spawns.doors.push({ x, y, open: false, secret: c === "S" });
      }
    }
  }
